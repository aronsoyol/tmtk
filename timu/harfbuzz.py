...
def loadlibrary_():
    ...
    if platform == "linux" or platform == "linux2":
        ...
        # OS X
        hb = ct.cdll.LoadLibrary("libharfbuzz.dylib")
    elif platform == "win32":
        # Windows...
        hb = ct.cdll.LoadLibrary("libharfbuzz.dll")
...
class HARFBUZZ:
    ...
    def to_position_t(x): return round(x * 64)

    def from_position_t(x): return x / 64
        ...
        if len(args) == 4:
            ...
            if isinstance(arg, (bytes, bytearray)):
                ...
                if len(args) != 4 or not all(i < 128 for i in args):
                    raise TypeError(
                        "TAG string must be 4 ASCII chars in [0 .. 255]")
                #end if
                c1, c2, c3, c4 = args
            else:
    ...
    unicode_compose_func_t = \
        ct.CFUNCTYPE(bool_t, ct.c_void_p, codepoint_t, codepoint_t,
                     ct.POINTER(codepoint_t), ct.c_void_p)
    unicode_decompose_func_t = \
        ct.CFUNCTYPE(bool_t, ct.c_void_p, codepoint_t, ct.POINTER(
            codepoint_t), ct.POINTER(codepoint_t), ct.c_void_p)
    unicode_decompose_compatibility_func_t = \
        ct.CFUNCTYPE(ct.c_uint, ct.c_void_p, codepoint_t,
                     ct.POINTER(codepoint_t), ct.c_void_p)
    ...
    reference_table_func_t = ct.CFUNCTYPE(
        ct.c_void_p, ct.c_void_p, tag_t, ct.c_void_p)
    ...
    font_get_nominal_glyph_func_t = ct.CFUNCTYPE(
        bool_t, ct.c_void_p, ct.c_void_p, codepoint_t, ct.POINTER(codepoint_t), ct.c_void_p)
    font_get_variation_glyph_func_t = ct.CFUNCTYPE(
        bool_t, ct.c_void_p, ct.c_void_p, codepoint_t, codepoint_t, ct.POINTER(codepoint_t), ct.c_void_p)
    ...
    font_get_glyph_origin_func_t = ct.CFUNCTYPE(bool_t, ct.c_void_p, ct.c_void_p, codepoint_t, ct.POINTER(
        position_t), ct.POINTER(position_t), ct.c_void_p)
    ...
    font_get_glyph_extents_func_t = ct.CFUNCTYPE(
        bool_t, ct.c_void_p, ct.c_void_p, codepoint_t, ct.POINTER(glyph_extents_t), ct.c_void_p)
    font_get_glyph_contour_point_func_t = ct.CFUNCTYPE(
        bool_t, ct.c_void_p, ct.c_void_p, codepoint_t, ct.c_uint, ct.POINTER(position_t), ct.POINTER(position_t), ct.c_void_p)
    font_get_glyph_name_func_t = ct.CFUNCTYPE(
        bool_t, ct.c_void_p, ct.c_void_p, codepoint_t, ct.POINTER(ct.c_char), ct.c_uint, ct.c_void_p)
    font_get_glyph_from_name_func_t = ct.CFUNCTYPE(bool_t, ct.c_void_p, ct.c_void_p, ct.POINTER(
        ct.c_char), ct.c_int, ct.POINTER(codepoint_t), ct.c_void_p)
...
def def_struct_class(name, ctname, conv=None, extra=None, init_defaults=None):
    ...
    class result_class:
        ...
        # to forestall typos
        __slots__ = tuple(field[0] for field in ctstruct._fields_)
        ...
        def __init__(self, *args, **kwargs):
            ...
            if len(unrecognized) != 0:
                raise TypeError("unrecognized fields for %s: %s" %
                                (name, unrecognized))
            #end if
        #end __init__
...
def def_callback_wrapper(celf, method_name, docstring, callback_field_name, destroy_field_name, def_wrap_callback_func, hb_proc):
    ...
    def set_callback(self, callback_func, user_data, destroy):
        ...
        getattr(hb, hb_proc)(self._hbobj,
                             wrap_callback_func, None, wrap_destroy)
    #end set_callback
...
hb.hb_version.argtypes = (ct.POINTER(ct.c_uint), ct.POINTER(
    ct.c_uint), ct.POINTER(ct.c_uint))
...
hb.hb_variation_from_string.argtypes = (
    ct.c_char_p, ct.c_int, ct.POINTER(HB.variation_t))
hb.hb_variation_to_string.restype = None
hb.hb_variation_to_string.argtypes = (
    ct.POINTER(HB.variation_t), ct.c_char_p, ct.c_uint)
...
hb.hb_unicode_compose.argtypes = (
    ct.c_void_p, HB.codepoint_t, HB.codepoint_t, ct.POINTER(HB.codepoint_t))
hb.hb_unicode_decompose.restype = HB.bool_t
hb.hb_unicode_decompose.argtypes = (ct.c_void_p, HB.codepoint_t, ct.POINTER(
    HB.codepoint_t), ct.POINTER(HB.codepoint_t))
hb.hb_unicode_decompose_compatibility.restype = ct.c_uint
hb.hb_unicode_decompose_compatibility.argtypes = (
    ct.c_void_p, HB.codepoint_t, ct.POINTER(HB.codepoint_t))

hb.hb_blob_create.restype = ct.c_void_p
hb.hb_blob_create.argtypes = (
    ct.c_void_p, ct.c_uint, ct.c_uint, ct.c_void_p, ct.c_void_p)
...
hb.hb_buffer_add_codepoints.argtypes = (
    ct.c_void_p, ct.c_void_p, ct.c_int, ct.c_uint, ct.c_int)
hb.hb_buffer_set_content_type.restype = None
hb.hb_buffer_set_content_type.argtypes = (
    ct.c_void_p, HB.buffer_content_type_t)
...
hb.hb_buffer_set_cluster_level.argtypes = (
    ct.c_void_p, HB.buffer_cluster_level_t)
...
hb.hb_buffer_set_segment_properties.argtypes = (
    ct.c_void_p, ct.POINTER(HB.segment_properties_t))
...
hb.hb_buffer_get_glyph_positions.argtypes = (
    ct.c_void_p, ct.POINTER(ct.c_uint))
...
hb.hb_buffer_serialize_glyphs.argtypes = (ct.c_void_p, ct.c_uint, ct.c_uint, ct.c_void_p, ct.c_uint, ct.POINTER(
    ct.c_uint), ct.c_void_p, HB.tag_t, ct.c_uint)
hb.hb_buffer_deserialize_glyphs.restype = HB.bool_t
hb.hb_buffer_deserialize_glyphs.argtypes = (
    ct.c_void_p, ct.c_void_p, ct.c_int, ct.c_void_p, ct.c_void_p, HB.tag_t)
...
hb.hb_font_get_scale.argtypes = (
    ct.c_void_p, ct.POINTER(ct.c_int), ct.POINTER(ct.c_int))
...
hb.hb_font_get_ppem.argtypes = (
    ct.c_void_p, ct.POINTER(ct.c_uint), ct.POINTER(ct.c_uint))
hb.hb_font_get_h_extents.restype = HB.bool_t
hb.hb_font_get_h_extents.argtypes = (
    ct.c_void_p, ct.POINTER(HB.font_extents_t))
hb.hb_font_get_v_extents.restype = HB.bool_t
hb.hb_font_get_v_extents.argtypes = (
    ct.c_void_p, ct.POINTER(HB.font_extents_t))
...
hb.hb_font_get_glyph_h_origin.argtypes = (
    ct.c_void_p, HB.codepoint_t, ct.POINTER(HB.position_t), ct.POINTER(HB.position_t))
...
hb.hb_font_get_extents_for_direction.argtypes = (
    ct.c_void_p, HB.direction_t, ct.POINTER(HB.font_extents_t))
hb.hb_font_get_glyph_advance_for_direction.restype = None
hb.hb_font_get_glyph_advance_for_direction.argtypes = (
    ct.c_void_p, HB.codepoint_t, HB.direction_t, ct.POINTER(HB.position_t), ct.POINTER(HB.position_t))
...
hb.hb_font_get_glyph_kerning_for_direction.argtypes = (
    ct.c_void_p, HB.codepoint_t, HB.codepoint_t, HB.direction_t, ct.POINTER(HB.position_t), ct.POINTER(HB.position_t))
hb.hb_font_get_glyph_extents_for_origin.restype = HB.bool_t
hb.hb_font_get_glyph_extents_for_origin.argtypes = (
    ct.c_void_p, HB.codepoint_t, HB.direction_t, ct.POINTER(HB.glyph_extents_t))
hb.hb_font_get_glyph_contour_point_for_origin.restype = HB.bool_t
hb.hb_font_get_glyph_contour_point_for_origin.argtypes = (
    ct.c_void_p, HB.codepoint_t, ct.c_uint, HB.direction_t, ct.POINTER(HB.position_t), ct.POINTER(HB.position_t))
...
hb.hb_shape_full.argtypes = (
    ct.c_void_p, ct.c_void_p, ct.c_void_p, ct.c_uint, ct.c_void_p)
...
hb.hb_ot_layout_get_glyphs_in_class.argtypes = (
    ct.c_void_p, HB.ot_layout_glyph_class_t, ct.c_void_p)
hb.hb_ot_layout_get_attach_points.restype = ct.c_uint
hb.hb_ot_layout_get_attach_points.argtypes = (
    ct.c_void_p, HB.codepoint_t, ct.c_uint, ct.POINTER(ct.c_uint), ct.POINTER(ct.c_uint))
hb.hb_ot_layout_get_ligature_carets.restype = ct.c_uint
hb.hb_ot_layout_get_ligature_carets.argtypes = (
    ct.c_void_p, HB.direction_t, HB.codepoint_t, ct.c_uint, ct.POINTER(ct.c_uint), ct.POINTER(HB.position_t))
hb.hb_ot_layout_table_get_script_tags.restype = ct.c_uint
hb.hb_ot_layout_table_get_script_tags.argtypes = (
    ct.c_void_p, HB.tag_t, ct.c_uint, ct.POINTER(ct.c_uint), ct.POINTER(HB.tag_t))
hb.hb_ot_layout_table_find_script.restype = HB.bool_t
hb.hb_ot_layout_table_find_script.argtypes = (
    ct.c_void_p, HB.tag_t, HB.tag_t, ct.POINTER(ct.c_uint))
hb.hb_ot_layout_table_choose_script.restype = HB.bool_t
hb.hb_ot_layout_table_choose_script.argtypes = (ct.c_void_p, HB.tag_t, ct.POINTER(
    HB.tag_t), ct.POINTER(ct.c_uint), ct.POINTER(HB.tag_t))
...
hb.hb_ot_layout_script_get_language_tags.argtypes = (
    ct.c_void_p, HB.tag_t, ct.c_uint, ct.c_uint, ct.POINTER(ct.c_uint), ct.POINTER(HB.tag_t))
hb.hb_ot_layout_script_find_language.restype = HB.bool_t
hb.hb_ot_layout_script_find_language.argtypes = (
    ct.c_void_p, HB.tag_t, ct.c_uint, HB.tag_t, ct.POINTER(ct.c_uint))
...
hb.hb_ot_layout_language_get_required_feature_index.argtypes = (
    ct.c_void_p, HB.tag_t, ct.c_uint, ct.c_uint, ct.POINTER(ct.c_uint))
...
hb.hb_ot_layout_language_get_feature_indexes.argtypes = (
    ct.c_void_p, HB.tag_t, ct.c_uint, ct.c_uint, ct.c_uint, ct.POINTER(ct.c_uint), ct.POINTER(HB.tag_t))
...
hb.hb_ot_layout_language_find_feature.argtypes = (
    ct.c_void_p, HB.tag_t, ct.c_uint, ct.c_uint, HB.tag_t, ct.POINTER(ct.c_uint))
hb.hb_ot_layout_feature_get_lookups.restype = ct.c_uint
hb.hb_ot_layout_feature_get_lookups.argtypes = (
    ct.c_void_p, HB.tag_t, ct.c_uint, ct.c_uint, ct.POINTER(ct.c_uint), ct.POINTER(ct.c_uint))
...
hb.hb_ot_layout_collect_lookups.argtypes = (ct.c_void_p, HB.tag_t, ct.POINTER(
    HB.tag_t), ct.POINTER(HB.tag_t), ct.POINTER(HB.tag_t), ct.c_void_p)
hb.hb_ot_layout_lookup_collect_glyphs.restype = None
hb.hb_ot_layout_lookup_collect_glyphs.argtypes = (
    ct.c_void_p, HB.tag_t, ct.c_uint, ct.c_void_p, ct.c_void_p, ct.c_void_p, ct.c_void_p)
...
hb.hb_ot_layout_get_size_params.argtypes = (ct.c_void_p, ct.POINTER(ct.c_uint), ct.POINTER(
    ct.c_uint), ct.POINTER(ct.c_uint), ct.POINTER(ct.c_uint), ct.POINTER(ct.c_uint))

hb.hb_ot_tags_from_script.restype = None
hb.hb_ot_tags_from_script.argtypes = (
    HB.script_t, ct.POINTER(HB.tag_t), ct.POINTER(HB.tag_t))
...
hb.hb_ot_shape_plan_collect_lookups.argtypes = (
    ct.c_void_p, HB.tag_t, ct.c_void_p)

hb.hb_shape_plan_create.restype = ct.c_void_p
hb.hb_shape_plan_create.argtypes = (ct.c_void_p, ct.POINTER(
    HB.segment_properties_t), ct.c_void_p, ct.c_uint, ct.c_void_p)
hb.hb_shape_plan_create_cached.restype = ct.c_void_p
hb.hb_shape_plan_create_cached.argtypes = (ct.c_void_p, ct.POINTER(
    HB.segment_properties_t), ct.c_void_p, ct.c_uint, ct.c_void_p)
...
hb.hb_shape_plan_execute.argtypes = (
    ct.c_void_p, ct.c_void_p, ct.c_void_p, ct.c_void_p, ct.c_uint)
...
if hasattr(hb, "hb_ot_math_has_data"):
    ...
    hb.hb_ot_math_get_glyph_italics_correction.argtypes = (
        ct.c_void_p, HB.codepoint_t)
    ...
    hb.hb_ot_math_is_glyph_extended_shape.argtypes = (
        ct.c_void_p, HB.codepoint_t)
    hb.hb_ot_math_get_glyph_kerning.restype = HB.position_t
    hb.hb_ot_math_get_glyph_kerning.argtypes = (
        ct.c_void_p, HB.codepoint_t, HB.ot_math_kern_t, HB.position_t)
    hb.hb_ot_math_get_glyph_variants.restype = ct.c_uint
    hb.hb_ot_math_get_glyph_variants.argtypes = (ct.c_void_p, HB.codepoint_t, HB.direction_t, ct.c_uint, ct.POINTER(
        ct.c_uint), ct.POINTER(HB.ot_math_glyph_variant_t))
    hb.hb_ot_math_get_min_connector_overlap.restype = HB.position_t
    hb.hb_ot_math_get_min_connector_overlap.argtypes = (
        ct.c_void_p, HB.direction_t)
    hb.hb_ot_math_get_glyph_assembly.restype = ct.c_uint
    hb.hb_ot_math_get_glyph_assembly.argtypes = (ct.c_void_p, HB.codepoint_t, HB.direction_t, ct.c_uint, ct.POINTER(
        ct.c_uint), ct.POINTER(HB.ot_math_glyph_part_t), ct.POINTER(HB.position_t))
#end if
...
if hasattr(hb, "hb_ot_var_get_axis_count"):
    ...
    hb.hb_ot_var_get_axes.argtypes = (ct.c_void_p, ct.c_uint, ct.POINTER(
        ct.c_uint), ct.POINTER(HB.ot_var_axis_t))
    hb.hb_ot_var_find_axis.restype = HB.bool_t
    hb.hb_ot_var_find_axis.argtypes = (
        ct.c_void_p, HB.tag_t, ct.POINTER(ct.c_uint), ct.POINTER(HB.ot_var_axis_t))
    hb.hb_ot_var_normalize_variations.restype = None
    hb.hb_ot_var_normalize_variations.argtypes = (ct.c_void_p, ct.POINTER(
        HB.variation_t), ct.c_uint, ct.POINTER(ct.c_int), ct.c_uint)
    hb.hb_ot_var_normalize_coords.restype = None
    hb.hb_ot_var_normalize_coords.argtypes = (
        ct.c_void_p, ct.c_uint, ct.POINTER(ct.c_float), ct.POINTER(ct.c_int))
#end if
...
def direction_to_string(direction):
    return \
        hb.hb_direction_to_string(
            direction).decode()  # automatically stops at NUL?
    ...
    def to_string(self):
        ...
        return \
            hb.hb_language_to_string(
                self._hbobj).decode()  # automatically stops at NUL?
    #end to_string
...
class UnicodeFuncs:
    ...
    def get_default():
        return \
            UnicodeFuncs(hb.hb_unicode_funcs_reference(
                hb.hb_unicode_funcs_get_default()))
    #end get_default
    ...
    def parent(self):
        return \
            UnicodeFuncs(hb.hb_unicode_funcs_reference(
                hb.hb_unicode_funcs_get_parent(self._hbobj)))
    #end parent
    ...
    for basename, def_func, protostr, resultstr in \
            (
                ("combining_class", def_wrap_combining_class_func,
                 "combining_class_func(self, unicode, user_data)", "n integer combining class code"),
                ("eastasian_width", def_wrap_eastasian_width_func,
                 "eastasian_width_func(self, unicode, user_data)", "n integer"),
                ("general_category", def_wrap_general_category_func,
                 "general_category_func(self, unicode, user_data)", "n integer general category code"),
                ("mirroring", def_wrap_mirroring_func,
                 "mirroring_func(self, unicode, user_data)", " unicode code point"),
                ("script", def_wrap_script_func,
                 "script_func(self, unicode, user_data)", "n integer script code"),
                ("compose", def_wrap_compose_func,
                 "compose_func(self, a, b, user_data)", " unicode code point or None"),
                ("decompose", def_wrap_decompose_func, "decompose_func(self, ab, user_data)",
                 " 2-tuple of Unicode code points or None"),
                ("decompose_compatibility", def_wrap_decompose_compatibility_func,
                 "decompose_compatibility_func(self, u, user_data)", " tuple of Unicode code points or None"),
            ):
...
class Blob:
    ...
    def create(celf, data, length, mode, user_data, destroy):
        ...
        if destroy != None:
            ...
            wrap_destroy = None
        #end if
        result = celf(hb.hb_blob_create(
            data, length, mode, None, wrap_destroy))
    ...
    def create_for_array(celf, arr, mode):
        ...
        if isinstance(arr, bytes):
            ...
            raise TypeError(
                "data is not bytes, bytearray or array.array of bytes")
        #end if
    ...
    def data(self, writable):
        ...
        length = ct.c_uint()
        addr = (hb.hb_blob_get_data, hb.hb_blob_get_data_writable)[
            writable](self._hbobj, ct.byref(length))
        if addr == None:
...
class Buffer:
    ...
    def segment_properties(self, segment_properties):
        ...
        #end if
        hb.hb_buffer_set_segment_properties(
            self._hbobj, segment_properties.to_hb())
    #end segment_properties
    ...
    def unicode_funcs(self):
        return \
            UnicodeFuncs(hb.hb_unicode_funcs_reference(
                hb.hb_buffer_get_unicode_funcs(self._hbobj)))
    #end unicode_funcs
    ...
    def glyph_positions(self):
        ...
        return \
            tuple(GlyphPosition.from_hb(arr[i], self.autoscale)
                  for i in range(nr_glyphs.value))
    #end glyph_positions

    if qahirah != None:
        def get_glyphs_codepoint(self):
            glyph_infos = self.glyph_infos
            return [glyph.codepoint for glyph in self.glyph_infos]
    ...
    def replacement_codepoint(self, replacement_codepoint):
        hb.hb_buffer_set_replacement_codepoint(
            self._hbobj, replacement_codepoint)
    #end replacement_codepoint
    ...
    def serialize_glyphs(self, font, format, flags):
        ...
        result = b""
        while pos != end:
            seg_items = hb.hb_buffer_serialize_glyphs(self._hbobj, pos, end, tempbuf.buffer_info()[0], tempbuf.buffer_info(
            )[1], ct.byref(buf_consumed), (lambda: None, lambda: font._hbobj)[font != None](), format._tag, flags)
...
class Face:
    ...
    def create_for_tables(reference_table, user_data, destroy, autoscale):
        ...
        result = Face(hb.hb_face_create_for_tables(
            wrap_reference_table, None, wrap_destroy), autoscale)
    ...
    def ot_layout_choose_script(self, table_tag, script_tags):
        ...
        success = hb.hb_ot_layout_table_choose_script(
            self._hbobj, table_tag, c_script_tags, script_index, chosen_script) != 0
        return \
    ...
    def ot_layout_script_get_language_tags(self, table_tag, script_index):
        ...
        while True:
            # just get the length on the first pass
            nr_language_tags = hb.hb_ot_layout_script_get_language_tags(self._hbobj, table_tag, script_index,
                                                                        0, language_count, language_tags)
    ...
    def ot_layout_script_find_language(self, table_tag, script_index, language_tag):
        ...
        success = hb.hb_ot_layout_script_find_language(
            self._hbobj, table_tag, script_index, language_tag, language_index) != 0
        return \
    ...
    def ot_layout_language_get_required_feature(self, table_tag, script_index, language_index):
        ...
        success = hb.hb_ot_layout_language_get_required_feature(
            self._hbobj, table_tag, script_index, language_index, feature_index, feature_tag) != 0
        return \
    ...
    def ot_layout_language_get_feature_indexes(self, table_tag, script_index, language_index):
        ...
        while True:
            # just get the length on the first pass
            nr_feature_indexes = hb.hb_ot_layout_language_get_feature_indexes(self._hbobj, table_tag, script_index,
                                                                              language_index, 0, feature_count, feature_indexes)
    ...
    def ot_layout_language_get_feature_tags(self, table_tag, script_index, language_index):
        ...
        while True:
            # just get the length on the first pass
            nr_feature_tags = hb.hb_ot_layout_language_get_feature_tags(self._hbobj, table_tag, script_index,
                                                                        language_index, 0, feature_count, feature_tags)
            if feature_tags != None:
                break
            # allocate space, now I know how much I need
            feature_count = ct.c_uint(nr_feature_tags)
    ...
    def ot_layout_language_find_feature(self, table_tag, script_index, language_index, feature_tag):
        feature_index = ct.c_uint()
        success = hb.hb_ot_layout_language_find_feature(
            self._hbobj, table_tag, script_index, language_index, feature_tag, feature_index) != 0
        return \
    ...
    def ot_layout_feature_get_lookups(self, table_tag, feature_index):
        ...
        while True:
            # just get the length on the first pass
            nr_lookups = hb.hb_ot_layout_feature_get_lookups(self._hbobj, table_tag, feature_index,
                                                             0, lookup_count, lookup_indexes)
    ...
    def ot_layout_collect_lookups(self, table_tag, scripts, languages, features):
        ...
        hb.hb_ot_layout_collect_lookups(
            self._hbobj, table_tag, c_scripts, c_languages, cC_features, lookup_indexes._hbobj)
        return \
    ...
    def ot_layout_lookup_collect_glyphs(self, table_tag, lookup_index, want_glyphs_before, want_glyphs_input, want_glyphs_after, want_glyphs_output):
        ...
        hb.hb_ot_layout_lookup_collect_glyphs(
            self._hbobj, table_tag, lookup_index, glyphs_before._hbobj, glyphs_input._hbobj, glyphs_after._hbobj, glyphs_output._hbobj)
        return \
            (glyphs_before.from_hb(), glyphs_input.from_hb(),
             glyphs_after.from_hb(), glyphs_output.from_hb())
    ...
    if hasattr(hb, "hb_ot_var_get_axis_count"):
        ...
        def ot_var_normalize_variations(self, variations):
            if (
                ...
                raise TypeError(
                    "variations must be sequence of Variation objects")
            #end if
            nr_variations = len(variations)
            c_variations = seq_to_ct(
                variations, HB.variation_t, Variation.to_hb)
...
class Font:
    ...
    def get_glyph_h_origin(self, glyph):
        ...
        if hb.hb_font_get_glyph_h_origin(self._hbobj, glyph, ct.byref(x), ct.byref(y)) != 0:
            if self.autoscale:
                result = (HB.from_position_t(x.value),
                          HB.from_position_t(y.value))
            else:
    ...
    def get_glyph_h_kerning(self, left_glyph, right_glyph):
        result = hb.hb_font_get_glyph_h_kerning(
            self._hbobj, left_glyph, right_glyph)
        if self.autoscale:
            result = HB.from_position_t(result)
        #end if
        return \
    ...
    def get_glyph_v_kerning(self, top_glyph, bottom_glyph):
        result = hb.hb_font_get_glyph_v_kerning(
            self._hbobj, top_glyph, bottom_glyph)
        if self.autoscale:
            result = HB.from_position_t(result)
        #end if
        return \
    ...
    def set_funcs(self, funcs, font_data, destroy):
        ...
        hb.hb_font_set_funcs(self._hbobj, (lambda: None, lambda: funcs._hbobj)[
                             funcs != None](), self._hbobj, wrap_destroy)
    #end set_funcs
    ...
    def parent(self, new_parent):
        ...
        #end if
        hb.hb_font_set_parent(self._hbobj, (lambda: None, lambda: new_parent._hbobj)[
                              new_parent != None]())
    #end parent
    ...
    def face(self):
        return \
            Face(hb.hb_face_reference(
                hb.hb_font_get_face(self._hbobj)), self.autoscale)
    ...
    if freetype != None:
        ...
        def ft_create(ft_face):
            ...
            if not isinstance(ft_face, freetype.Face):
                raise TypeError("ft_face must be a freetype.Face")
            #end if
            result = Font(hb.hb_ft_font_create_referenced(
                ft_face._ftobj), True)
    ...
    if hasattr(hb, "hb_ot_math_has_data"):
        ...
        def ot_math_get_glyph_top_accent_attachment(self, glyph):
            result = hb.hb_ot_math_get_glyph_top_accent_attachment(
                self._hbobj, glyph)
            if self.autoscale:
                result = HB.from_position_t(result)
            #end if
        ...
        def ot_math_get_min_connector_overlap(self, direction):
            "direction is a DIRECTION_XXX value."
            result = hb.hb_ot_math_get_min_connector_overlap(
                self._hbobj, direction)
            if self.autoscale:
                result = HB.from_position_t(result)
            #end if
            return \
        ...
        def ot_math_get_glyph_assembly(self, glyph, direction):
            ...
            if self.autoscale:
                italics_correction = HB.from_position_t(
                    c_italics_correction.value)
            else:
...
#begin def_font_extra
    for propname, proptype, docextra, conv_to_hb, conv_from_hb in \
            (
                ("ppem", ct.c_uint,
                 "A zero value means no hinting in that direction.", None, None),
...
class FontFuncs:
    ...
    def get_empty():
        ...
        return \
            FontFuncs(hb.hb_font_funcs_reference(
                hb.hb_font_funcs_get_empty()), False)
    #end get_empty
...
def def_fontfuncs_extra():
    ...
    def def_wrap_get_nominal_glyph_func(wself, get_nominal_glyph, user_data):
        ...
        def wrap_get_nominal_glyph(c_font, c_font_data, unicode, c_glyph, c_user_data):
            font = get_font(c_font)
            glyph = get_nominal_glyph(
                font, font._font_data, unicode, user_data)
            if glyph != None:
                c_glyph[0] = glyph
            #end if
            return \
    ...
    def def_wrap_get_glyph_extents_func(wself, get_glyph_extents, user_data):
        ...
        def wrap_get_glyph_extents(c_font, c_font_data, glyph, c_extents, c_user_data):
            font = get_font(c_font)
            extents = get_glyph_extents(
                font, font._font_data, glyph, user_data)
            if extents != None:
    ...
    for basename, def_func, protostr, resultstr in \
            (
                ("font_h_extents", def_wrap_get_font_extents_func,
                 "get_font_h_extents(font, font_data, user_data)", " FontExtents or None"),
                ("font_v_extents", def_wrap_get_font_extents_func,
                 "get_font_v_extents(font, font_data, user_data)", " FontExtents or None"),
                ("nominal_glyph", def_wrap_get_nominal_glyph_func,
                 "get_nominal_glyph(font, font_data, unicode, user_data)", "n integer glyph code or None"),
                ("variation_glyph", def_wrap_get_variation_glyph_func,
                 "get_variation_glyph(font, font_data, unicode, variation_selector, user_data)", "n integer glyph code or None"),
                ("glyph_h_advance", def_wrap_get_glyph_advance_func,
                 "get_glyph_h_advance(font, font_data, glyph, user_data)", " float"),
                ("glyph_v_advance", def_wrap_get_glyph_advance_func,
                 "get_glyph_v_advance(font, font_data, glyph, user_data)", " float"),
                ("glyph_h_origin", def_wrap_get_glyph_origin_func,
                 "get_glyph_h_origin(font, font_data, glyph, user_data)", " 2-tuple, qahirah.Vector or None"),
                ("glyph_v_origin", def_wrap_get_glyph_origin_func,
                 "get_glyph_v_origin(font, font_data, glyph, user_data)", " 2-tuple, qahirah.Vector or None"),
                ("glyph_h_kerning", def_wrap_get_glyph_kerning_func,
                 "get_glyph_h_kerning(font, font_data, first_glyph, second_glyph, user_data)", " float"),
                ("glyph_v_kerning", def_wrap_get_glyph_kerning_func,
                 "get_glyph_v_kerning(font, font_data, first_glyph, second_glyph, user_data)", " float"),
                ("glyph_extents", def_wrap_get_glyph_extents_func,
                 "get_glyph_extents(font, font_data, glyph, user_data)", " GlyphExtents or None"),
                ("glyph_contour_point", def_wrap_get_glyph_contour_point_func,
                 "get_glyph_contour_point(font, font_data, glyph, point_index, user_data)", " 2-tuple, qahirah.Vector or None"),
                ("glyph_name", def_wrap_get_glyph_name_func,
                 "get_glyph_name_func(font, font_data, glyph, user_data)", " string or None"),
                ("glyph_from_name", def_wrap_get_glyph_from_name_func,
                 "get_glyph_from_name(font, font_data, name, user_data)", "n integer glyph code or None"),
            ):
        def_callback_wrapper(
            celf=FontFuncs,
            method_name="set_%s_func" % basename,
...
def shape_full(font, buffer, features=None, shaper_list=None):
    ...
    result = hb.hb_shape_full(
        font._hbobj, buffer._hbobj, c_features, nr_features, c_shaper_list) != 0
    buffer.autoscale = font.autoscale
    return \
        result
#end shape_full
...
def shape_list_shapers():
    c_shaper_list = ct.cast(hb.hb_shape_list_shapers(),
...
def ot_tags_from_script(script):
    ...
    hb.hb_ot_tags_from_script(script, ct.byref(
        script_tag_1), ct.byref(script_tag_2))
    return \
...
class ShapePlan:
    ...
    def create(face, props, user_features, shaper_list, cached):
        ...
        return \
            ShapePlan(
                ...
                (face._hbobj, c_props, c_user_features,
                 nr_user_features, c_shaper_list)
            )
    #end create
    ...
    def execute(self, font, buffer, features):
        ...
        if features != None:
            ...
            nr_features = 0
        #end if
        result = hb.hb_shape_plan_execute(
            self._hbobj, font._hbobj, buffer._hbobj, c_features, nr_features) != 0
        buffer.autoscale = font.autoscale
        return \
            result
    #end execute
    ...
    def ot_collect_lookups(self, table_tag):
        hb_set = Set.to_hb()
        hb.hb_ot_shape_plan_collect_lookups(
            self._hbobj, table_tag, hb_set._hbobj)
        return \
            hb_set.from_hb()
